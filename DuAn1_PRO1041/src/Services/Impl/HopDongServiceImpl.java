...
public class HopDongServiceImpl implements IHopDongService {
    ...
    @Override
    public String updateHDTS(HopDongTaiSan hdts, String idTS, int slCu) {
        boolean update = hdri.updateHDTS(hdts, idTS, slCu);
        if (update) {
            return "Sửa Hợp Đồng Tài Sản thành công!";
        } else {
            return "Thêm Hợp Đồng Tài Sản thất bại!";
        }
    }

    @Override
    public HopDongTaiSan getHDTSByIdHD_IdTS(String idHD, String idTS) {
        return hdri.getHDTSByIdHD_IdTS(idHD, idTS);
    }
