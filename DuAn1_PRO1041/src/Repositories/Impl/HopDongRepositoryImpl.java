...
public class HopDongRepositoryImpl implements IHopDongRepository {
    ...
    @Override
    public boolean updateHDTS(HopDongTaiSan hdts, String idTS, int slCu) {
        int check = 0;
        String query = """
                       UPDATE [dbo].[Hop_dong_ts]
                          SET [IdTaiSan] = ?
                             ,[soLuong] = ?
                        WHERE [IdTaiSan] = ? 
        ...
        try (Connection con = DBConnect.getConnection();
            ...
            ps.setObject(1, hdts.getIdTS());
            ps.setObject(2, hdts.getSoLuong());
            ps.setObject(3, idTS);
            ps.setObject(4, hdts.getSoLuong());
            ps.setObject(5, slCu);
            ps.setObject(6, idTS);
    ...
    @Override
    public HopDongTaiSan getHDTSByIdHD_IdTS(String idHD, String idTS) {
        String query = """
                       SELECT dbo.Hop_dong_ts.Id, dbo.Hop_dong_ts.IDHopDong, dbo.Hop_dong_ts.IdTaiSan, dbo.Hop_dong_ts.soLuong
                       FROM     dbo.Hop_dong INNER JOIN
                                         dbo.Hop_dong_ts ON dbo.Hop_dong.Id = dbo.Hop_dong_ts.IDHopDong INNER JOIN
                                         dbo.Tai_san ON dbo.Hop_dong_ts.IdTaiSan = dbo.Tai_san.Id
                       WHERE dbo.Hop_dong.Id = ? AND  dbo.Tai_san.Id = ?
                       """;
        try (Connection con = DBConnect.getConnection();
                PreparedStatement ps = con.prepareStatement(query)) {
            ps.setObject(1, idHD);
