            ...
            if (!idKH.equalsIgnoreCase(hd.getIdKH())) {
                ...
                if (ma.equalsIgnoreCase(hd.getMa())) {
                    if (checkData()) {
                        ...
                        for (HopDongTaiSan hdts : listHDTS) {
//                            if (!service.getListHDTSByIdHD(id).contains(hdts)) {
//                                hdts.setId(String.valueOf(UUID.randomUUID()));
//                                hdts.setIdHD(id);
//                                service.addHDTS(hdts);
//                            } else {
                            service.updateHDTS(hdts, hdts.getIdTS(), service.getHDTSByIdHD_IdTS(id, hdts.getIdTS()).getSoLuong());
//                            }
                        }
                        showDataAll();
                    }
    ...
    private void tblListHDMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblListHDMouseClicked
        ...
        listHDTS = service.getListHDTSByIdHD(idHD);
...
//                if (listTS.get(index).getSoLuong() <= 0) {
//                    JOptionPane.showMessageDialog(this, "Tài sản này đã hết!");
//                    return;
//                }
                dem++;
